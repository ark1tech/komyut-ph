...
- [x] New component: `src/lib/components/map/NavigationSearchBar.svelte`.
- [x] Two inputs using same `ILIKE` search as existing `MapSearchBar`.
- [x] On submit → calls `GET /api/navigate` → shows `RouteLoaderBar`.

### 5b. Pre-navigation Result Display
**In `Map.svelte`:**
- [x] Render Walk legs: dashed gray line.
- [x] Render Ride legs: solid colored clipped LineString.
- [x] Bottom card: step-by-step itinerary + summary row.
- [x] "Start Navigation" button.

### 5c. Live Navigation Mode
- [x] `watchPosition` → `map.easeTo({ center, bearing })` on each fix.
- [x] Bearing from `GeolocationCoordinates.heading` or computed from last two positions.
- [x] Zoom 17, pitch 45°.
- [x] Re-center button when user pans away.
- [x] Current leg highlighted; completed legs dimmed.

### 5d. "I've arrived" & Combined Route Save
- [x] Ends navigation, returns camera to north-up.
- [x] If ≥2 ride legs: prompt to save combined route.
- [x] Pre-fills `RouteTraceSaveDrawer`: concatenated geometry, auto `mode_segments`, merged `vehicle_types`, summed fares + ETAs.
