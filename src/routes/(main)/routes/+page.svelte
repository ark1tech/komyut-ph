<script lang="ts">
	...
	// Assumption (temporary): until we have actual usage tracking + persistence,
	// these are believable subsets of the mock routes.
	const recentRoutes: SavedRoute[] = mockRoutes.slice(0, 6);
	const savedRoutes: SavedRoute[] = mockRoutes.slice(6, 12);
	...
	const activeList = $derived(view === 'recent' ? recentRoutes : savedRoutes);
</script>
...
<div class="sticky top-0 z-30 border-b bg-background/95 backdrop-blur-sm">
	<div class="relative flex items-center justify-between gap-fluid-sm px-fluid-sm py-fluid-sm">
		<div class="min-w-9" aria-hidden="true" />
...
<section class="space-y-3 px-fluid-sm py-fluid-sm" role="region" aria-label="Routes">
	...
	<div class="flex items-center justify-between">
		...
		{#if view === 'recent'}
			<a href="/map" class="text-xs font-medium text-muted-foreground hover:text-foreground">
				Open map
			</a>
		{:else}
			<a href="/profile/savedroutes" class="text-xs font-medium text-muted-foreground hover:text-foreground">
				Manage
			</a>
		{/if}
	</div>
	...
	{:else}
		{#each activeList as route (route.route_id)}
			<article class="rounded-2xl bg-card p-4 transition-colors hover:bg-accent">
				<a href="/map?route={route.route_id}" class="block">
